...
import { 
  ...
  DollarSign, RefreshCw, Filter, MessageCircle, Eye, Info, List
} from 'lucide-react';
...
const Overdue = () => {
  ...
  const getSnowballDetails = (loan: Loan) => {
      ...
      while (tempDue < today) {
          ...
          const updatedVal = calculateOverdueValue(baseAmount, dateStr, 'Atrasado', loan.fineRate ?? 2, loan.moraInterestRate ?? 1, loan.amount);
      ...
      if (missedInstallments.length === 0 && loan.status === 'Atrasado') {
           ...
           const updatedVal = calculateOverdueValue(baseAmount, dateStr, 'Atrasado', loan.fineRate ?? 2, loan.moraInterestRate ?? 1, loan.amount);
           missedInstallments.push({ date: dateStr, original: baseAmount, updated: updatedVal });
           totalOriginal += baseAmount;
           totalUpdated += updatedVal;
      }
  ...
  const handleWhatsApp = (loan: Loan, snowball: any) => {
      ...
      const parcelasText = snowball.missedInstallments.length > 1 ? `${snowball.missedInstallments.length} parcelas pendentes` : `uma pendência`;
      
      const message = `Olá ${firstName}, identificamos ${parcelasText} totalizando R$ ${formatMoney(snowball.totalUpdated)} (valor atualizado) referente ao seu contrato.\n\nPodemos agendar um pagamento para regularizar?`;
    ...
    <Layout>
      <header className="flex justify-between items-center mb-8">
        <div>
          ...
          <p className="text-slate-500">Gestão de contratos em atraso e recuperação (Efeito Bola de Neve).</p>
        </div>
        <button 
            onClick={fetchData} 
                            ...
                            return (
                                <tr key={loan.id} className="hover:bg-red-50/30 transition-colors group">
                                    ...
                                    <td className="p-4 text-right flex items-center justify-end gap-2">
                                        ...
                                        <button 
                                            onClick={() => handleWhatsApp(loan, loan.snowball)}
                                            className="text-green-600 hover:bg-green-100 hover:text-green-800 p-2 rounded-lg transition-all flex items-center gap-1 font-bold text-xs border border-green-200 shadow-sm bg-white"
                                            title="Enviar mensagem de cobrança"
                                        >
                                            <Phone size={14}/> Cobrar
                                        </button>
                                    </td>
                                </tr>
                            );
              ...
              <div className="space-y-6">
                  <div className="bg-red-50 p-5 rounded-2xl border border-red-200 shadow-inner">
                      ...
                      <div className="flex gap-4 border-t border-red-200/50 pt-4">
                          <div className="flex-1 bg-white p-3 rounded-xl border border-red-100 text-center">
                              <span className="block text-[10px] uppercase font-bold text-slate-400 mb-1">Qtd. Parcelas</span>
                              <span className="text-lg font-black text-slate-800">{selectedSnowball.snowball.missedInstallments.length}</span>
                          </div>
                          <div className="flex-1 bg-white p-3 rounded-xl border border-red-100 text-center">
                              <span className="block text-[10px] uppercase font-bold text-slate-400 mb-1">Valor Original</span>
                              <span className="text-lg font-black text-slate-800">R$ {formatMoney(selectedSnowball.snowball.totalOriginal)}</span>
                          </div>
                          <div className="flex-1 bg-white p-3 rounded-xl border border-red-100 text-center">
                              <span className="block text-[10px] uppercase font-bold text-slate-400 mb-1">Juros / Multa Gerados</span>
                  ...
                  <div className="pt-2">
                          ...
                          <table className="w-full text-left text-sm">
                              <thead className="bg-slate-100 text-[10px] uppercase text-slate-500">
                                  <tr>
                                      ...
                                      <th className="p-3 text-right text-red-500">Mora/Multa</th>
                                      ...
                                      return (
                                          <tr key={idx} className="hover:bg-white transition-colors">
                                              <td className="p-3 font-bold text-slate-700">
                                                  {new Date(inst.date + 'T12:00:00').toLocaleDateString('pt-BR')}
                                              </td>
                                              <td className="p-3 text-right text-slate-500">R$ {formatMoney(inst.original)}</td>
                                              <td className="p-3 text-right text-red-500 font-bold">+ R$ {formatMoney(juros)}</td>
                      ...
                      <div className="flex items-start gap-2 mt-3 p-3 bg-blue-50 text-blue-800 rounded-lg text-xs">
                          ...
                          <p>O cálculo leva em consideração a multa fixa de <b>{selectedSnowball.fineRate}%</b> e mora de <b>{selectedSnowball.moraInterestRate}% ao dia</b> sobre cada parcela individualmente.</p>
                      </div>
                  </div>

                  <div className="pt-4 border-t border-slate-100 flex justify-end gap-3">
                      <button onClick={() => setIsModalOpen(false)} className="px-6 py-3 text-slate-600 font-bold hover:bg-slate-100 rounded-xl transition-all">Fechar Radiografia</button>
                      <button 
                          onClick={() => handleWhatsApp(selectedSnowball, selectedSnowball.snowball)}
